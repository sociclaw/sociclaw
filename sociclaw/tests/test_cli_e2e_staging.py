...
def test_cli_e2e_staging_minimal_success(tmp_path, monkeypatch, capsys):
...
def test_cli_e2e_staging_fails_without_user_state(tmp_path, monkeypatch, capsys):
    config_path = tmp_path / "runtime.json"
    RuntimeConfigStore(config_path).save(RuntimeConfig(provider="telegram", provider_user_id="123"))
    monkeypatch.setenv("SOCICLAW_PROVISION_URL", "https://api.sociclaw.com/api/sociclaw/provision")
    monkeypatch.setenv("XAI_API_KEY", "x-key")
    ...
    parser = build_parser()
    args = parser.parse_args(["e2e-staging", "--config-path", str(config_path)])
    rc = args.func(args)
    assert rc == 1
    payload = json.loads(capsys.readouterr().out)
    assert payload["status"] == "failed"
    assert payload["required_failures"] >= 1
